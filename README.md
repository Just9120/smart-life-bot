...
- **OAuth Slice 6.1/6.2/6.3a — реализованы только foundation/boundary срезы**: есть UX-каркас `🔐 Личный Google Calendar`, callback boundary skeleton и token exchange/persistence boundary skeleton, но нет callback-сервера, реального Google code/token exchange, реального persistence access/refresh tokens и personal-calendar writes.
...
- Weekday parsing: `вторник 10:30` = ближайший будущий вторник; если сегодня вторник, токен `вторник` означает следующий вторник (для текущего дня используйте `сегодня`).
...
- В `📋 Активные категории` карточка записи открывает menu: `⚙️ Действия №N` → `✏️ Изменить процент` / `🗑 Удалить` / `❌ Закрыть` (edit поддерживает только процент; delete = soft-delete).
