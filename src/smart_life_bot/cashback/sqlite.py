...
class SQLiteCashbackCategoriesRepository:
    ...
    def upsert(self, payload: CashbackAddInput) -> tuple[CashbackCategoryRecord, bool, float | None]:
        ...
        if existing is not None:
            ...
            self._connection.execute(
                "UPDATE cashback_categories SET percent=?, source_text=?, updated_at=?, category_raw=? WHERE id=?",
                (payload.percent, payload.source_text, now, payload.category_raw, existing["id"]),
            )
    ...
    def query(self, category_key: str, target_month: str) -> list[CashbackCategoryRecord]:
        rows = self._connection.execute(
            ...
            (normalize_category_key(category_key), target_month),
        ).fetchall()
        return [self._to_record(r) for r in rows]

    def _to_record(self, row) -> CashbackCategoryRecord:
        return CashbackCategoryRecord(
            id=row["id"], owner_name=row["owner_name"], bank_name=row["bank_name"], category_raw=row["category_raw"], category_key=row["category_key"],
            percent=float(row["percent"]), target_month=row["target_month"], source_text=row["source_text"],
            created_at=_parse_iso_datetime(row["created_at"]), updated_at=_parse_iso_datetime(row["updated_at"]), is_deleted=bool(row["is_deleted"]),
        )
