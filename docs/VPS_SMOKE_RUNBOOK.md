...
16. Отправьте owner-first multi-add, например: `Владимир, Т-Банк, Супермаркеты 5%, Аптеки 5%`, и проверьте summary по пунктам `1.`, `2.`.
17. Проверьте negative-case: >5 пар категорий возвращает `Можно добавить не больше 5 категорий за раз.` и не пишет частично.
18. Проверьте, что multi-add не создаёт calendar draft и не затрагивает calendar callbacks.
