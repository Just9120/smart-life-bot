...
7. Проверьте явные действия в меню cashback: `📋 Активные категории`, `➕ Добавить категорию`, `🔎 Найти категорию`, `📤 Экспорт XLSX`.
8. Нажмите `📤 Экспорт XLSX`: бот открывает picker месяца (с навигацией), затем нажмите `✅ Выгрузить этот месяц`.
9. Проверьте export outcomes: при наличии данных за выбранный месяц бот отправляет `.xlsx`; при пустом месяце — дружелюбный no-data ответ без файла.
10. Экспорт read-only: после выгрузки записи не должны меняться; destructive DB cleanup запрещён.
11. Нажмите `➕ Добавить категорию` и проверьте, что callback-действие работает в реальном Telegram (не только transport tests).
   Проверьте подсказку формата: одиночный add + owner-first multi-add (с запятыми и без).
12. Нажмите `🔎 Найти категорию` и проверьте, что callback-действие работает в реальном Telegram (не только transport tests).
13. В режиме `💳 Кэшбек` отправьте plain text категории и проверьте default query/search routing.
14. Проверьте deterministic aliases в `💳 Кэшбек`: `продукты`/`еда`→`Супермаркеты`, `лекарства`/`медицина`→`Аптеки`, `бензин`/`топливо`→`АЗС`.
15. Проверьте deterministic query/search variants Sprint 5 slice 2:
    - `супермаркет` / `магазины продуктов` → `Супермаркеты`;
    - `аптека` → `Аптеки`;
    - `заправка` / `заправки` / `а-з-с` → `АЗС`.
    Ожидание: это узкий детерминированный search-only слой; broad fuzzy matching и LLM fallback в этом срезе не ожидаются; при таких поисковых запросах calendar draft не создаётся и вызовов Google Calendar нет.
16. Нажмите `📋 Активные категории` и проверьте видимую нумерацию строк `1.`, `2.` (без `#1`, `#2`).
17. Проверьте, что кнопки Edit/Delete применяются к тем же видимым номерам строк.
18. Проверьте owner reset: `Все` / `✅ Все` корректно снимает фильтр владельца.
19. Переключите режимы `📅 Календарь` ↔ `💳 Кэшбек` и проверьте, что несовместимые pending-состояния очищаются.
20. Data caution (production): Telegram smoke в `💳 Кэшбек` пишет в реальную persisted SQLite DB. Используйте реалистичные записи и удаляйте тестовые через UI (Edit/Delete); не выполняйте destructive DB cleanup в рамках smoke.
21. Отправьте owner-first multi-add, например: `Владимир, Т-Банк, Супермаркеты 5%, Аптеки 5%`, и проверьте summary по пунктам `1.`, `2.`.
