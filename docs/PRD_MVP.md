...
Правило mode-routing в MVP:
- footer-кнопки `📅 Календарь` / `💳 Кэшбек` выбирают активный режим;
- бот явно показывает текущий режим (`Текущий режим: ...`) при выборе раздела;
- свободный текст интерпретируется только внутри активного режима;
- если активный режим не выбран и пришёл неоднозначный plain text, бот не угадывает и просит: `Выбери режим: 📅 Календарь или 💳 Кэшбек.`
- исключение MVP: явный структурный cashback add (например `банк, владелец, категория, процент`) принимается глобально как не-двусмысленный command-like ввод.
...
- В `📋 Активные категории` доступен owner-filter по whitelist-владельцам (`Виктор`, `Владимир`, `Елена`) и сброс на `Все владельцы`; фильтр применяется к выбранному месяцу.
