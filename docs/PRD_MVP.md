...
- Формат структурного добавления поддерживает оба варианта: `банк, владелец, категория, процент` и короткий `банк владелец категория процент`.
  - В переходный период конца месяца при добавлении без явного месяца бот показывает inline-выбор месяца (`Текущий` / `Следующий`) и выполняет добавление только после явного выбора месяца.
  - Явный месяц также поддерживается напрямую в тексте: `банк, владелец, месяц, категория, процент`.
