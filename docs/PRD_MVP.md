...
- Формат структурного добавления: `банк, владелец, категория, процент`.
  - В переходный период конца месяца для валидного 4-part ввода бот показывает inline-кнопки выбора месяца (текущий/следующий) и завершает добавление без повторного ввода всей строки.
  - Явный формат с месяцем продолжает работать: `банк, владелец, месяц, категория, процент`.
