...
- cashback query;
- strict mode-routing: ambiguous plain text без активного режима возвращает запрос выбора режима (`📅` или `💳`) без guessing;
- strict mode-routing: в `💳 Кэшбек` query-not-found не проваливается в calendar preview;
- strict mode-routing: в `📅 Календарь` короткие фразы (`Аптеки`, `Созвон`, `Тренировка`) идут в calendar draft/missing-date path, не в cashback query;
- mode switch safety: `📅 Календарь` очищает pending cashback edit/transition state, `💳 Кэшбек` очищает pending calendar date-recovery state;
