...
- cashback owner-first multi-add (comma + space variants, optional month token);
...
- deterministic cashback query variants (`супермаркет`, `магазины продуктов`, `аптека`, `заправка/заправки`) и boundary negative-cases без broad fuzzy matching;
...
- owner-first multi-add: лимит 5 категорий, invalid pair => no writes (all-or-nothing), без calendar side effects.
...
- `📤 Экспорт XLSX` открывает picker месяца, а не сразу отправляет файл;
- `✅ Выгрузить этот месяц` экспортирует только выбранный месяц (без записей из других месяцев);
- пустой выбранный месяц возвращает дружелюбный no-data ответ и не отправляет пустой файл;
- экспорт read-only: записи cashback не меняются, calendar drafts/calls не создаются;
- `.xlsx` остаётся структурно валидным и содержит форматирование (title/header, freeze pane, autofilter, widths, deterministic sorting);
...
- Нажатие `✅ Создать событие` создаёт calendar event только при явном действии пользователя.
...
- regression guard: OAuth stub actions не создают calendar writes.

Следующие пункты ниже относятся к future slices (6.2+) и остаются pending.

После внедрения `oauth_user_mode` regression-набор должен покрывать минимум:
- missing-auth behavior в personal mode (явный guidance connect flow);
- connect/disconnect/status state transitions;
- stale/invalid OAuth callback state (CSRF-safe reject);
- token refresh failure handling (`provider_auth_failure` + reconnect-required UX);
- provider auth failure mapping без утечки token/secret details в user text/logs;
- confirm invariant: calendar write только после `✅ Создать событие`/`draft:confirm`;
...
## 8) Future screenshot cashback parser coverage (backlog, not implemented)

Эти требования относятся к будущему backlog-кандидату и **не означают текущую реализацию**.

Минимальный future coverage scope:
- screenshot extraction возвращает только draft-кандидаты (preview stage), без немедленной записи в SQLite;
- malformed/irrelevant image безопасно даёт `no safe extraction` (или equivalent) без auto-save;
- проценты из нерелевантных блоков (promo/credit/deposit/banner) отфильтровываются и не попадают в candidate cashback list;
- model output с невалидными/подозрительно высокими процентами (например, выше будущего configurable threshold, например `30%`) отбрасывается или явно флагируется;
- stop-word фильтры (`кредит`, `вклад`, `ставка`, `страховка`, `заём`, `рассрочка`) применяются до preview/save;
- отсутствуют calendar side effects (не создаются drafts/events календаря);
- в логах нет raw screenshot bytes и нет full sensitive Vision payload dump;
- запись в storage выполняется только после explicit user confirmation (`Сохранить`), а `Отменить` ничего не пишет.


## Актуальные regression-фокусы (коротко)

- OAuth callback boundary tests (Slice 6.2): state validation/reuse rejection + safe error mapping.
- OAuth token boundary/redaction tests (Slice 6.3a): redaction-safe models/use-case outcomes без real token persistence.
- Context Bundle Builder tests: safety exclusions и защита от попадания служебных/секретных путей в bundles.
- Calendar weekday parsing tests: `вторник` как future-weekday modifier, `сегодня` как explicit current-day token, weekday token не остаётся в title.
- Cashback tests: `📋 Активные категории` actions menu, percent-only edit flow, owner-first multi-add (включая `т банк`), delete soft-delete path.
