...
- CD workflow `Deploy VPS` (`.github/workflows/deploy.yml`) запускается автоматически на `push` в `main` и также поддерживает ручной запуск через `workflow_dispatch`.
...
## 8. CD workflow (auto + manual, hardened guardrails)

Workflow: `Deploy VPS` (`.github/workflows/deploy.yml`)

- Trigger: `push` only for branch `main` (auto-deploy after merge) + manual `workflow_dispatch`.

- После merge PR в `main` ожидаемый сигнал успешного релиза: workflow `Deploy VPS` доходит до маркера `deploy_phase=runtime_verification_ok` в логах run.
